#include "SiliconOnlyTrackResiduals.h"
...
TrackResiduals::process_event (
  ...
  Gl1RawHit* gl1_raw_hit     = findNode::getClass<Gl1RawHit>(topNode, "GL1RAWHIT");
  Gl1Packet* gl1_packet_info = findNode::getClass<Gl1Packet>(topNode, "GL1RAWHIT");

  if (gl1_raw_hit) {
    m_bco = gl1_raw_hit->get_bco();
...
TrackResiduals::fillFailedSeedTree (
  ...
  if (!trackmap or !clustermap or !silseedmap or !svtxseedmap or !geometry)
  {
    std::cout << "Missing node, can't continue" << std::endl;
    return;
  }
